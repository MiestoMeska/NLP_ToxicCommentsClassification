...
class Main_Model_Multihead_Shared(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        binary_output = self.binary_classifier(shared_output)

        toxic_output = self.toxic_classifier(shared_output)
        severe_toxic_output = self.severe_toxic_classifier(shared_output)
        obscene_output = self.obscene_classifier(shared_output)
        threat_output = self.threat_classifier(shared_output)
        insult_output = self.insult_classifier(shared_output)
        identity_hate_output = self.identity_hate_classifier(shared_output)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if toxic_samples.any():
            ...
            multilabel_preds = [torch.sigmoid(multilabel_output[i][toxic_indices]) > 0.5 for i in range(self.num_labels)]
